...
- **ack 驱动的带宽裁剪**：每个数据报回带本端已连续收到的对端最高帧（`ack`）；发送端据此
  只重发对端【尚未确认】的帧（保底留最新一帧），不再无脑重发满 W 帧。实测省 **~79% 带宽**
  （发出帧数 717 vs 无裁剪基线 3344），confirmed 轨迹仍逐位一致——省带宽不损正确性。
  `ack` 陈旧（其自身可能丢/迟）只会多发几帧、绝不少发，故对丢包恢复安全。`Windower` 是这套
  「冗余 + 去重 + ack 裁剪」的单一实现，线上 `ClientTransport` 与进程内测试信道共用。
- **连接质量统计（`ConnStats`）**：不依赖墙钟、可确定性测试。`RttFrames = 最新本地帧 − peerAck`
  是往返延迟的帧数估计（本端帧走单程 L 步、ack 再走 L 步回来，故 ≈ 2L）——实测 L=2→5 帧、
  L=6→13 帧，随延迟单调增大。另有 `StaleSteps`（连续无新远端帧的步数）作掉线信号：对家静默
  20 步后逼近 20。`ClientTransport.Stats()` 暴露给 `cmd/client`（已在进度日志打印）与将来的
  C# 客户端做延迟显示 / 自适应 / 掉线判定。
- **输入延迟自适应（`DelayController`）**：把实测 `RttFrames` 经 EWMA 平滑，推荐输入延迟
  `D ≈ round(RTT/2)`（单程延迟）——这是「输入延迟 + 回滚」混合（GGPO/SF6 家族）的调参核心：
  用最小的输入延迟覆盖多数网络延迟。测试：稳态 RTT→D 收敛、上下限夹取、抖动下 EWMA 稳住不跳；
  再喂【实测】RTT（L=2→RTT 5→D 3，L=6→RTT 13→D 6）验证端到端。**诚实边界**：当前 Go
  `RollbackPeer` 的 D 是「缓冲提前」（`simFrame=wallTick+D`），对称移动收发帧号，故不改变回滚
  窗口（≈L）；要用 D 减少**可见**回滚，需客户端把显示帧退到预测头之后——属客户端集成范畴。
  `DelayController` 只做推荐（开局/局间采用，两端各选不破坏确定性；局中改需按约定帧同步切换）。
- **断线检测与重连**：客户端在握手时生成稳定 `client_id`（`JoinRequest.client_id`）；服务器按
  【身份】而非【地址】认座位，故换 socket / NAT 重绑 / 服务器重启后，凭同一 `client_id` re-Join
  即重新映射回原座位、输入转发改路由到新地址（旧地址不再收到）。客户端由 `StaleSteps` 阈值判连接
  状态（连接中 / 已连接 / 断流 / 掉线），非「已连接」时心跳重发握手自动重连。全程只在传输层发生，
  不触碰 confirmed 轨迹——断流期间模拟照常预测，恢复后追上。测试：`classifyState` 阈值 + 真 UDP
  下「同 `client_id` 从新地址重连 → 服务器把对家输入改投到新地址」。
